...
<div class="row p-2">
    ...
    <div class="col-md-4 text-center form-group">
        <input type="text" id="searchInput2" class="form-control" onkeyup="search()" placeholder="Email bo'yicha qidiring..">
    </div>
    ...
    <div class="col-md-2 text-center">
        <a href="add-user.php"><button class="btn btn-success">Yangi qo'shish</button></a>
    </div>
</div>
        ...
        <?php
            ...
            if($data != null){
                foreach($data as $id => $arr){
                    $full_name = $arr['name']." ".$arr['surname'];
                    echo "
                    
                        <tr>
                            <td class='td'>{$counter}</td>
                            <td class='td'>{$full_name}</td>
                            <td class='td'>{$arr['email']}</td>
                            <td class='td'>{$arr['password']}</td>
                            <td class='td'>
                                <a href='update-user.php?id={$id}' class='btn btn-warning text-light'>
                            ...
                            <td class='td'>
                                <a href='delete-user.php?id={$id}' class='btn btn-danger'>
