...
<div class="row p-2">
    ...
    <div class="col-md-4 text-center form-group">
        <input type="text" id="searchInput1" class="form-control" onkeyup="search()" placeholder="Nom bo'yicha qidiring..">
    </div>
    ...
    <div class="col-md-2 text-center">
        <a href="add-lesson.php"><button class="btn btn-success">Yangi qo'shish</button></a>
    </div>
</div>
            ...
            if($data != null){
                foreach($data as $id => $arr){
                    ...
                    if($arr['status'] == 'published'){
                        $my_class = "bg-success text-light p-2 rounded";
                    }if($arr['status'] == 'draft'){
                        $my_class = "bg-secondary text-light p-2 rounded";
                    }
                    echo "
                    
                        <tr>
                            <td class='td'>{$counter}</td>
                            <td class='td'>{$arr['name']}</td>
                            <td class='td'>{$arr['topic']}</td>
                            <td class='td'><a href='#' class='{$my_class}'>{$arr['status']}</a></td>
                            <td class='td'>
                                <a href='update-lesson.php?id={$id}' class='btn btn-warning text-light'>
                            ...
                            <td class='td'>
                                <a href='delete-lesson.php?id={$id}' class='btn btn-danger'>
