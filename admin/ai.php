...
<div class="add-form container p-5">
    <form action="add-ai-action.php" method="POST" enctype="multipart/form-data" class="bg-primary shadow rounded p-3">
        <h4 class="text-center text-light">Detal qo'shish</h4>
        <input type="text" name="ai_name" placeholder="Detal nomi" class="form-control mb-2">
                ...
                foreach($data as $id => $arr){
                    echo "
                        <tr>
                            ...
                            <td class='td'>{$arr['ai_name']}</td>
                            <td class='td'>{$arr['ai_file']}</td>
                            <td class='td'>
                                <a href='delete-ai.php?id={$id}&file={$arr['ai_file']}' class='btn btn-danger'>
