...
$data = $db->retrieve("mt_files/{$got_id}");
            ...
            <div class='form-group container shadow rounded bg-primary p-3'>
                <div class='row p-2'>
                    <div class='col-md-4 p-3 bg-light rounded'>
                        <h4 class=''>{$data['mt_name']}</h4>
                        <a href='../mt/{$data['mt_file']}' download class='btn btn-success'>Yuklab olish</a>
                    </div>
                    <div class='col-md-4'>
                        <form action='upload-mt-work.php' method='POST' enctype='multipart/form-data' class='shadow rounded bg-light p-2'>
                            <input type='hidden' name='mt_id' value='{$got_id}'>
...
$data1 = $db->retrieve("mt_works");
$data1 = json_decode($data1, 1);
$filename = "";
$found_id = "";
if($data1 != null){
    foreach ($data1 as $id1 => $arr1){
        if($arr1['mt_id'] = $got_id && $arr1['user_id'] = $_SESSION['id']){
            $found_id = $id1;
            $filename = $arr1['mt_file'];
        }
    }
}
...
if($found_id != "" && $filename != ""){
    echo "
        <div class='col-md-4 p-3 bg-light rounded'>
            <span class='text-dark mb-2'>{$filename}</span><br><a href='delete-mt-work.php?id={$found_id}&file={$filename}' class='btn btn-danger mt-2'>O'chirish</a>
        </div>
    ";
}
